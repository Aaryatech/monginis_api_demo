...
public interface FrSubCatItemReportNewRepo extends JpaRepository<FrSubCatItemReportNew, Integer> {
	...
	@Query(value = " SELECT * FROM( SELECT\r\n" + 
			...
			" 	 COALESCE(t1.sold_qty,0)-(COALESCE(t3.var_qty, 0)+COALESCE(t2.ret_qty, 0)) as net_qty,\r\n" + 
			"    COALESCE(t1.sold_amt,0)-(COALESCE(t3.var_amt, 0)+COALESCE(t2.ret_amt, 0)) as net_amt,\r\n" + 
			"    ((COALESCE(t3.var_amt, 0)+COALESCE(t2.ret_amt, 0))*100)/COALESCE(t1.sold_amt,0) as ret_amt_per " +
			"FROM\r\n" + 
			"    (\r\n" + 
			"    SELECT\r\n" + 
			...
			"    item_name) t3\r\n" + 
			"ON\r\n" + 
			"    t1.fr_id = t3.fr_id AND t1.item_id = t3.item_id ) t ORDER BY fr_name,cat_id,sub_cat_id,item_name ", nativeQuery = true)
	List<FrSubCatItemReportNew> getFrWiseSubCatWiseItemWiseData(@Param("fromDate") String fromDate, @Param("toDate") String toDate,
