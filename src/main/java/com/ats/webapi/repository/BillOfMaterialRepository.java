...
public interface BillOfMaterialRepository extends JpaRepository<BillOfMaterialHeader, Integer> {
	 ...
	 * 
	 * @Query(
	 * value="select * from t_req_bom where req_date BETWEEN :frmdate and :todate and del_status=0 "
	 * + "and from_dept_id=:bmsDeptId or from_dept_id=:mixDeptId",nativeQuery=true)
	 * List<BillOfMaterialHeader> getAlllist(@Param("frmdate")String
	 * frmdate,@Param("todate")String todate, @Param("bmsDeptId")int
	 * bmsDeptId, @Param("mixDeptId")int mixDeptId );
	 * 
	 * @Query(value=" select * from t_req_bom where  req_id=:reqId and del_status=0"
	 * ,nativeQuery=true) BillOfMaterialHeader getByReqstId(@Param("reqId")int
	 * reqId);
	 * 
	 * //@Query(value=" select * from t_req_bom where del_status=0",nativeQuery=
	 * true) List<BillOfMaterialHeader>
	 * findBillOfMaterialHeaderByDelStatusOrderByReqIdDesc(int delStatus);
	 * 
	 * 
	 * @Query(
	 * value=" select * from t_req_bom where  req_date BETWEEN :frmdate and :todate and from_dept_id=:fromDept and to_dept_id=:toDept and del_status=0"
	 * ,nativeQuery=true) List<BillOfMaterialHeader>
	 * getAlllist(@Param("fromDept")int fromDept, @Param("toDept")int
	 * toDept, @Param("frmdate")String frmdate, @Param("todate")String todate);
	 * 
	 * @Query(
	 * value=" select * from t_req_bom where  status IN(:status) and from_dept_id=:fromDept and to_dept_id IN (:toDept) and del_status=0 order by req_id DESC"
	 * ,nativeQuery=true) List<BillOfMaterialHeader>
	 * getAlllist(@Param("fromDept")int fromDept, @Param("toDept")List<Integer>
	 * toDept, @Param("status")List<String> status);
	 * 
	 * 
	 * @Transactional
	 * 
	 ...
	 * ) int updateStatusWhileCompletProd(@Param("prodId")int
	 * prodId,@Param("isProduction")int isProduction);
	 * 
