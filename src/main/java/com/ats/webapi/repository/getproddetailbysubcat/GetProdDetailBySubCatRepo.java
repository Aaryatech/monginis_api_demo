...
public interface GetProdDetailBySubCatRepo extends JpaRepository<GetProdDetailBySubCat, Integer> {
	...
	/*
	 * @Query(
	 * value=" SELECT  subCat.sub_cat_name,subCat.sub_cat_id,item.item_grp2, CASE WHEN ph.is_planned=1 THEN SUM(pd.plan_qty) "
	 * +
	 ...
	 * +
	 * " and ph.production_header_id=pd.production_header_id and item.id=pd.item_id group by item.item_grp2"
	 * ,nativeQuery=true) List<GetProdDetailBySubCat>
	 * getProdDetailBySubCat(@Param("prodHeaderId")int prodHeaderId);
	 */
